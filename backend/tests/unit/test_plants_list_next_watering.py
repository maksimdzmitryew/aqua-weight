...
def test_next_watering_projection_and_roll_forward(monkeypatch):
    ...
    monkeypatch.setattr(
        pl_mod, "get_last_watering_event_since", lambda c, u: (last_watering_at, None, None, 100.0)
    )
...
def test_next_watering_projection_future_no_roll_forward(monkeypatch):
    ...
    monkeypatch.setattr(
        pl_mod, "get_last_watering_event_since", lambda c, u: (last_at, None, 100.0, 50.0)
    )
    items = PlantsList.fetch_all()
...
def test_next_watering_db_exception_yields_none(monkeypatch):
    ...
    class TwoCursorConn:
        ...
        def cursor(self):
            self.count += 1
            if self.count == 2:
                return FakeCursor(execute_raises=True)
            return cursor
...
def test_next_watering_vacation_mode_decay(monkeypatch):
    ...
    # Mock get_last_watering_event_since to return a tuple (measured_at, last_dry_weight_g, last_wet_weight_g, water_added_g)
    monkeypatch.setattr(
        pl_mod, "get_last_watering_event_since", lambda c, u: (last_at, None, None, 100.0)
    )
...
def test_check_watering_prediction_all_cases(monkeypatch):
    ...
    cursor.add_response(
        "SELECT measured_at FROM plants_measurements", [(now - timedelta(days=4),)]
    )  # last watering
    cursor.add_response(
        "SELECT measured_at, measured_weight_g",
        [(now - timedelta(days=2), 100), (now - timedelta(days=1), 99), (now, 98.5)],
    )
    monkeypatch.setattr(pl_mod, "get_conn", lambda: FakeConn(cursor))
    items = PlantsList.fetch_all()
...
def test_fetch_all_extended_branches(monkeypatch):
    ...
    monkeypatch.setattr(pl_mod, "get_conn", lambda: FakeConn(cursor))
    items = PlantsList.fetch_all(
        status="archived", search="50", sort_by="name", sort_dir="desc", limit=10, offset=0
    )
...
def test_count_all_comprehensive(monkeypatch):
    ...
    count = PlantsList.count_all(
        search="foo",
        status="active",
        needs_weighing_filter=False,
        current_user={"id": 1, "global_role": "user"},
    )
    assert count == 100
...
def test_fetch_all_get_last_watering_event_since_exception(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda c, u: (_ for _ in ()).throw(RuntimeError("boom")),
    )
...
def test_fetch_all_projection_timedelta_exception(monkeypatch):
    ...
    monkeypatch.setattr(
        pl_mod, "get_last_watering_event_since", lambda c, u: (last_watering_at, 100.0, 200.0, 50.0)
    )
...
def test_check_watering_prediction_fail_all(monkeypatch):
    ...
    cursor.add_response(
        "FROM plants_measurements WHERE plant_id = UNHEX(%s) AND measured_at > %s AND measured_weight_g IS NOT NULL",
        rows_m,
    )
    monkeypatch.setattr(pl_mod, "get_conn", lambda: FakeConn(cursor))
    items = PlantsList.fetch_all()
...
def test_fetch_all_search_text_and_exceptions(monkeypatch):
    ...
    cursor2 = FakeCursor(
        rows=[_full_row(pid, "P1", created=now, measured=now, weight=150, loss=0.0)]
    )
    ...
    cursor3 = FakeCursor(
        rows=[_full_row(pid, "P1", created=now, measured=now, weight=110, loss=20.0)]
    )  # retained (110-100)/50 = 20%
