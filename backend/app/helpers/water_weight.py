...
def update_min_dry_weight_and_max_watering_added_g(conn, plant_id_hex: str, new_measured_weight_g: Optional[int], new_added_watering_g: Optional[int]) -> None:
    ...
    try:
        ...
        if current_weight_min is None or (new_measured_weight_g is not None and new_measured_weight_g < current_weight_min):
            current_weight_min = new_measured_weight_g

        if current_watering_max is None or (new_added_watering_g is not None and new_added_watering_g > current_watering_max):
