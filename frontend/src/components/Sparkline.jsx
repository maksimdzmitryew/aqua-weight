...
export function computePeakVLines(points, maxWaterG, peakDeltaPct, dtPref) {
  const result = []
  const threshAbs =
    isFinite(maxWaterG) && maxWaterG > 0 && isFinite(peakDeltaPct) && peakDeltaPct > 0
...
export default function Sparkline({
  ...
  const refYs = (Array.isArray(refLines) ? refLines : [])
    .map((rl) => (rl && isFinite(rl.y) ? rl.y : null))
    .filter((v) => v != null)
  ...
  // Area path (optional)
  const area =
    points.length > 1 && defaultFill
      ? `${path} L${sx(points[points.length - 1].x)},${margin.top + h} L${sx(points[0].x)},${
          margin.top + h
        } Z`
      : ''
  ...
  const tooltipLines =
    hoverIdx != null
      ? typeof hoverFormatter === 'function'
        ? hoverFormatter(points[hoverIdx], hoverIdx, { points })
        : defaultHoverLines(hoverIdx)
      : []
  ...
  const contentWidth = wrappedLines.length
    ? Math.min(maxTooltipW - tooltipPadX * 2, Math.max(...wrappedLines.map(measureWidth)))
    : 0
  const tooltipWidth = wrappedLines.length
    ? Math.max(minTooltipW, contentWidth + tooltipPadX * 2)
    : 0
  const tooltipHeight = wrappedLines.length
    ? wrappedLines.length * tooltipLineHeight + tooltipPadY * 2
    : 0
  ...
  const firstBelowDaysSincePrevPeak =
    !hideFirstBelow && firstBelowThresh
  ...
  return (
    <div style={containerStyle} ref={containerRef}>
      ...
      >
            ...
            return (
              <g key={`ref-${idx}`}>
                ...
                {label && (
                  <text
                    x={margin.left + 4}
                    y={Math.max(margin.top + 8, Math.min(margin.top + h - 4, y - 2))}
                    fontSize={10}
                    fill={color}
                  >
                    {label}
        ...
        {area && <path d={area} fill={defaultFill} stroke="none" />}
        <path
          d={path}
          fill="none"
        ...
        {dotLast && last && <circle cx={sx(last.x)} cy={sy(last.y)} r={3} fill={defaultStroke} />}
      ...
      {htmlTip && (
        <div
          style={{
            ...
            boxShadow:
              effectiveTheme === 'dark' ? '0 2px 8px rgba(0,0,0,0.6)' : '0 2px 8px rgba(0,0,0,0.1)',
            pointerEvents: 'none',
