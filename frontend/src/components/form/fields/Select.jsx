...
export default function Select({ form, name, label, disabled, required, validators, children, ...rest }) {
  ...
  return (
    <div>
      ...
      <select id={name} disabled={disabled} aria-invalid={!!error} aria-describedby={error ? `${name}-error` : undefined} required={required} {...reg} {...rest} style={inputStyle}>
        {children}
      </select>
