...
export default function NumberInput({ form, name, label, placeholder, min, max, step, disabled, required, validators, ...rest }) {
  ...
  return (
    <div>
      ...
      <input id={name} type="number" placeholder={placeholder} min={min} max={max} step={step} disabled={disabled} aria-invalid={!!error} aria-describedby={error ? `${name}-error` : undefined} required={required} {...reg} {...rest} style={inputStyle} />
