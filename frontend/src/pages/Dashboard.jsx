...
export default function Dashboard() {
  ...
  return (
    <DashboardLayout title="Dashboard">
      ...
      <p>
        Each plant is represented by its weight trend since the last repotting. Daily compression
        shows only the last reading per day.
      </p>
      ...
      >
        <label style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <input
            type="checkbox"
            checked={showMinRef}
            onChange={(e) => setShowMinRef(e.target.checked)}
          />
        ...
        <label style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <input
            type="checkbox"
            checked={showMaxRef}
            onChange={(e) => setShowMaxRef(e.target.checked)}
          />
        ...
        <label style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <input
            type="checkbox"
            checked={showThreshRef}
            onChange={(e) => setShowThreshRef(e.target.checked)}
          />
      ...
      {!loading && !error && hasPlants && (
        <div
          style={{
            display: 'grid',
            gridTemplateColumns: `repeat(${chartsPerRow}, minmax(0, 1fr))`,
            gap: 16,
          }}
        >
          {plants.map((p, idx) => {
            ...
            const maxWater = Number.isFinite(p?.max_water_weight_g)
              ? Number(p.max_water_weight_g)
              : null
            const threshPct = Number.isFinite(p?.recommended_water_threshold_pct)
              ? Number(p.recommended_water_threshold_pct)
              : null
            if (showMinRef && minDry != null) refLines.push({ y: minDry, label: 'Dry' })
            if (showMaxRef && minDry != null && maxWater != null)
              ...
              >
                ...
                {seriesLoading && !pts.length ? (
                  ...
                  <Sparkline
                    ...
                    maxWaterG={
                      Number.isFinite(p?.max_water_weight_g) ? Number(p.max_water_weight_g) : null
                    }
