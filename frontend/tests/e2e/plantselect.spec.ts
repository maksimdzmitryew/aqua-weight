...
test.describe('PlantSelect (Watering Create page)', () => {
  test('shows loading, then populates options on success', async ({ page }) => {
    ...
    await expect(
      page.locator('select#plant_id >> option', { hasText: 'Playwright Fern' }),
    ).toHaveCount(1)
    await expect(
      page.locator('select#plant_id >> option', { hasText: 'Playwright Ivy' }),
    ).toHaveCount(1)
  ...
  test('renders error state when names API fails (covers non-abort error path)', async ({
    page,
  }) => {
    // Force the names endpoint to fail with 500 to trigger component error path
    await page.route('**/api/plants/names', (route) =>
      route.fulfill({
        status: 500,
        contentType: 'application/json',
